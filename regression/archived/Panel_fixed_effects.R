...
run_within_weekly <- function(mobility_df,ACS_df,cases_df,policy_df,
  ...
  # removing max number of cases/deaths if desired
  if (no_max){
    tildes_df <- tildes_df %>% 
...
out_weekly_cont <- run_within_weekly(mobility,ACS,cases,policy,
                                     'stay_at_home_shelter_in_place',
                                     bin = FALSE)
fit_weekly_cont <- out_weekly_cont[[1]]
tildes_weekly_cont <- out_weekly_cont[[2]]
merged_all_weekly_cont <- out_weekly_cont[[3]]