...
state_policies <- read.csv("../data/covid_us_state_policies.csv")
...
state_mobility <- gather(state_mobility, date, mobility, X2020.02.16:X2020.03.29, factor_key=FALSE)
...
p0<-
   plot_ly(data=sd.state,x = ~date, y = ~mobility, height=750) %>% 
    ...
    add_trace(name="State of Emergency Declared", x= ~STATE.OF.EMERGENCY,mode='lines') %>% 
    add_lines(name="All states",data=state_mobility,x=~date,y=~mobility,
              colors="black",color=~STATE,alpha=.1,showlegend=F,hoverinfo="none") %>%
