...
function update() {
	...
	dataNew.forEach(function(d) {
		d.fips = +d.fips;
		rateById[d.fips] = +d.pred;
		countyById[d.fips] = d.county;
	});

	console.log(dataNew);

	d3.selectAll('path').data(dataNew).transition().duration(1000).style('fill', function(d) {
		if (!isNaN(rateById[d.fips])) {
			return color(rateById[d.fips]);
		}
	});
			...
			if (!isNaN(rateById[d.fips])) {
				...
				tooltip
					.html(
						'<p><strong>' +
							...
							// dateFunctionNoYear(d.properties.seconds[second][0].seconds) +
							'</td><td><b>' +
							formatPercent(rateById[d.fips] / 100) +
							'</b></td></tr>'
					)
					.style('left', d3.event.pageX + 15 + 'px')
					.style('top', d3.event.pageY - 28 + 'px');
				...
				tooltip
					.html(
						...
						// dateFunctionNoYear(d.properties.seconds[second][0].seconds) +
						'</td><td><b>' + 'No available data' + '</b></td></tr>'
					)
					.style('left', d3.event.pageX + 15 + 'px')
					.style('top', d3.event.pageY - 28 + 'px');
			}
...
function redo() {
	...
	d3.selectAll('path').data(dataOld).style('fill', 'white').transition().duration(1000).style('fill', function(d) {
		if (!isNaN(rateById[d.fips])) {
			return color(rateById[d.fips]);
		}
	});
